// Package approverpage serves the page a paired approver device runs (D43).
// The page is static and the same for every Companion: in direct mode the
// Companion serves it next to its own approver API, in relay mode the relay
// serves it and forwards only the API calls into the right tunnel. One copy
...
// The page is one file with no framework, the way the authorization page
// is: a phone opens it from a QR code, keeps it as a home-screen app, and
// everything it needs arrives with the first request.
//
//go:embed page.html
var pageHTML string
...
func randomToken(n int) string {
	...
	if _, err := rand.Read(buf); err != nil {
		panic(fmt.Sprintf("approverpage: reading random bytes: %v", err))
	}
